...
/* ═══════════════════════════════════════════════════════════════════════════
   REGION 2f: AUTH & TEAM MODEL — Build 2.1
   Enterprise foundation: users, team, roles, permissions
   → /src/constants/auth.js + /src/data/team-seed.js
   ═══════════════════════════════════════════════════════════════════════════ */

const ROLES = {
  director:  { label: "Director", color: T.GOLD, icon: "🎬", desc: "Full access — creative and technical decisions",
    can: { screenplay:"edit", console:"edit", storyboard:"edit", generate:true, review:"full", audio:"edit", timeline:"edit", team:"edit", budget:"view", statusGate:"all" }},
  writer:    { label: "Writer", color: T.CYAN, icon: "✍️", desc: "Screenplay editing, scene structure, dialogue",
    can: { screenplay:"edit", console:"view", storyboard:"edit", generate:false, review:"annotate", audio:"none", timeline:"none", team:"none", budget:"none", statusGate:"reviewed" }},
  producer:  { label: "Producer", color: T.PURPLE, icon: "📊", desc: "Approvals, budgets, timeline, team management",
    can: { screenplay:"view", console:"view", storyboard:"approve", generate:true, review:"full", audio:"view", timeline:"edit", team:"edit", budget:"edit", statusGate:"locked" }},
  colorist:  { label: "Colorist", color: T.TEAL, icon: "🎨", desc: "Color grading, camera settings, visual look",
    can: { screenplay:"none", console:"camera", storyboard:"none", generate:false, review:"annotate", audio:"none", timeline:"none", team:"none", budget:"none", statusGate:"none" }},
  auditor:   { label: "Film Auditor", color: T.ORANGE, icon: "👁️", desc: "Review output, annotate, approve or reject takes",
    can: { screenplay:"none", console:"none", storyboard:"none", generate:false, review:"full", audio:"none", timeline:"none", team:"none", budget:"none", statusGate:"none" }},
};

const TEAM_SEED = {
  id: "team_crushed_01",
  name: "Shadows Production Unit",
  plan: "studio", // free | creator | pro | studio | enterprise
  creditBalance: 4200,
  creditLimit: 5000,
...
const USERS_SEED = [
  { id: "u1", name: "Rishi Mehta", email: "rishi@crushedstudios.ai", avatar: "R", role: "director", color: T.GOLD, online: true },
  { id: "u2", name: "Priya Sharma", email: "priya@crushedstudios.ai", avatar: "P", role: "writer", color: T.CYAN, online: true },
  { id: "u3", name: "Arjun Kapoor", email: "arjun@crushedstudios.ai", avatar: "A", role: "producer", color: T.PURPLE, online: false },
  { id: "u4", name: "Nadia Chen", email: "nadia@external-agency.com", avatar: "N", role: "auditor", color: T.ORANGE, online: false },
];

const CREDIT_COSTS = {
  generateVideo: 100,    // per scene
  generateStoryboard: 20, // per panel set
  generateAudioTrack: 10, // per track
  regenerateVideo: 80,    // cheaper than first gen
};

const PROJECTS_SEED = () => [
  {
    id: "proj_shadows", title: "Shadows of Dharavi", logline: "A forensic accountant uncovers a money-laundering network that leads back to the one person she trusted most.",
    genre: "Noir", status: "active", createdAt: "2026-02-10T00:00:00Z", updatedAt: new Date().toISOString(),
    scenesTotal: 3, scenesLocked: 1, scenesGenerated: 1, thumbnail: "🎬",
    owner: "u1", collaborators: ["u1","u2","u3","u4"], creditUsed: 847,
  },
  {
    id: "proj_monsoon", title: "Monsoon Protocol", logline: "A climate scientist races to prove her atmospheric model before a superstorm makes it irrelevant.",
    genre: "Sci-Fi", status: "active", createdAt: "2026-03-01T00:00:00Z", updatedAt: "2026-03-18T14:30:00Z",
    scenesTotal: 7, scenesLocked: 4, scenesGenerated: 2, thumbnail: "🌧️",
    owner: "u1", collaborators: ["u1","u2"], creditUsed: 1420,
  },
  {
    id: "proj_raga", title: "The Last Raga", logline: "An aging sitar master takes on one final student — a deaf girl who can feel music through vibrations.",
    genre: "Drama", status: "completed", createdAt: "2026-01-20T00:00:00Z", updatedAt: "2026-02-28T09:00:00Z",
    scenesTotal: 12, scenesLocked: 12, scenesGenerated: 12, thumbnail: "🎵",
    owner: "u3", collaborators: ["u1","u2","u3"], creditUsed: 3200,
  },
  {
    id: "proj_neon", title: "Neon Bazaar", logline: "A street food vendor discovers her grandmother's recipes unlock memories stored in Mumbai's quantum network.",
    genre: "Fantasy", status: "archived", createdAt: "2025-11-05T00:00:00Z", updatedAt: "2025-12-20T00:00:00Z",
    scenesTotal: 5, scenesLocked: 3, scenesGenerated: 0, thumbnail: "✨",
    owner: "u2", collaborators: ["u1","u2"], creditUsed: 0,
  },
];

const PROJECT_TEMPLATES = [
  { id: "tpl_blank", name: "Blank Project", desc: "Empty canvas — start from scratch", icon: "📄", scenes: 1, acts: 1 },
  { id: "tpl_short", name: "Short Film (3 Act)", desc: "Classic 3-act structure, 3–5 scenes", icon: "🎬", scenes: 3, acts: 3 },
  { id: "tpl_commercial", name: "Commercial / Ad", desc: "30–60s single-scene spot", icon: "📺", scenes: 1, acts: 1 },
  { id: "tpl_music", name: "Music Video", desc: "Performance + narrative intercut", icon: "🎵", scenes: 4, acts: 2 },
  { id: "tpl_feature", name: "Feature Film", desc: "Full-length 3-act structure, 15+ scenes", icon: "🎞️", scenes: 15, acts: 3 },
];
...
const StatusPipeline = ({ currentStatus, onStatusChange, canAdvance, statusGate = "all" }) => {
  const currentIdx = STATUS_FLOW.indexOf(currentStatus);
  // statusGate limits how far this user can advance
  const gateIdx = statusGate === "all" ? STATUS_FLOW.length - 1 : statusGate === "none" ? -1 : STATUS_FLOW.indexOf(statusGate);
  return (
    <div style={{display:"flex",alignItems:"center",gap:0}}>
      {STATUS_FLOW.map((sk,i) => {
        ...
        const beyondGate = i > gateIdx;
        const canClick = !beyondGate && ((isNext && canAdvance) || isPast);
        return (
          <div key={sk} style={{display:"flex",alignItems:"center"}}>
            <button
              onClick={()=>{ if (canClick) onStatusChange(sk); }}
              ...
              style={{
                ...
                borderRadius:6,padding:"5px 12px",cursor:canClick||isActive?"pointer":"not-allowed",
                opacity:(isFuture&&!isNext)||beyondGate?0.3:1,transition:"all 0.2s",
              }}
            >
              ...
              {beyondGate && isNext && <span style={{fontFamily:M,fontSize:7,color:"#555"}}>🔒</span>}
            </button>
...
export default function App() {
  ...
  // Each project stores its own {bible, chars, scenes, screenplay}
  const projectDataStore = useRef({
    "proj_shadows": "LIVE", // sentinel: means "use current live state" — gets real data on first save
    "proj_monsoon": {
      bible: { genre: "Sci-Fi", era: "Near Future", colorGrade: "Teal & Orange", cameraBody: "RED V-RAPTOR XL 8K", lensType: "Anamorphic", filmStock: "Digital ACES", aspectRatio: "2.39:1 Anamorphic", focalLength: "50mm Standard" },
      chars: [
        { id:"cm1", name:"Dr. Ananya Rao", color:T.CYAN, physical:{Hair:"Black, tied back","Skin Tone":"Medium brown",Build:"Slim",Height:"5'4\"","Scars/Marks":"Burn mark on left hand","Facial Hair":"—","Age Range":"Mid 30s","Distinguishing Feature":"Always carries weather instruments"}, mannerisms:["Talks fast","Checks instruments","Paces when thinking","Stares at sky"], defaultEmotion:"Determination", loraRef:true, defaultWardrobe:"Lab coat over kurta, weathered boots" },
        { id:"cm2", name:"Major Vikram", color:T.PURPLE, physical:{Hair:"Buzz cut","Skin Tone":"Dark brown",Build:"Athletic",Height:"6'0\"","Scars/Marks":"Scar across jawline","Facial Hair":"Clean shaven","Age Range":"Early 40s","Distinguishing Feature":"Military bearing, never sits"}, mannerisms:["Stands at attention","Clipped speech","Checks watch","Eyes scan room"], defaultEmotion:"Skepticism", loraRef:false, defaultWardrobe:"Military fatigues, tactical vest" },
      ],
      scenes: [{ id:"sm1", number:1, title:"The Warning", location:"Climate Research Lab, Mumbai", time:"00:00–00:45", camera:{shotType:"Medium Shot",movement:"Handheld — Documentary",lighting:"Fluorescent",atmosphere:"Urgent"}, chars:{cm1:{emotion:"Anxiety",wardrobeOverride:null,mannerismOverride:null,dialogue:"The model shows landfall in 36 hours.",deliveryEmo:"Anxiety",blocking:"Standing at holographic display",continuityNotes:""}}, validated:true, issues:[], status:"draft", storyboard:{panels:[],version:0,generating:false}, takes:[], screenplay:{heading:"INT.",slugline:"CLIMATE RESEARCH LAB — DAY",action:"Screens flicker with red warning overlays. DR. ANANYA RAO stares at a holographic storm model spinning above the central console.",dialogueBlocks:[{charId:"cm1",line:"The model shows landfall in 36 hours. We need to evacuate the coast.",parenthetical:"pointing at the projection"}],transition:"CUT TO:",beat:"Inciting Incident",notes:"Establish urgency and Ananya's expertise.",duration:45} }],
      screenplay: { title:"Monsoon Protocol", logline:"A climate scientist races to prove her atmospheric model before a superstorm makes it irrelevant.", synopsis:"", acts:[{id:"am1",name:"Act I — Warning",scenes:["sm1"]}] },
    },
    "proj_raga": {
      bible: { genre: "Drama", era: "Contemporary", colorGrade: "Warm Vintage", cameraBody: "ARRI Alexa 35", lensType: "Spherical Prime", filmStock: "Kodak Vision3 200T", aspectRatio: "1.85:1 Flat", focalLength: "85mm Telephoto" },
      chars: [
        { id:"cr1", name:"Ustad Farid", color:T.GOLD, physical:{Hair:"White, long","Skin Tone":"Weathered olive",Build:"Frail",Height:"5'8\"","Scars/Marks":"Arthritis in fingers","Facial Hair":"Long white beard","Age Range":"Late 70s","Distinguishing Feature":"Never without his sitar"}, mannerisms:["Hums ragas","Closes eyes when listening","Taps fingers on surfaces","Long pauses before speaking"], defaultEmotion:"Serenity", loraRef:true, defaultWardrobe:"Simple white kurta, shawl" },
        { id:"cr2", name:"Meera (student)", color:T.TEAL, physical:{Hair:"Short, messy","Skin Tone":"Light brown",Build:"Small",Height:"4'11\"","Scars/Marks":"Hearing aids in both ears","Facial Hair":"—","Age Range":"14","Distinguishing Feature":"Touches surfaces to feel vibrations"}, mannerisms:["Places hands on walls","Watches lips carefully","Communicates via sign + expression","Sits on floor cross-legged"], defaultEmotion:"Curiosity", loraRef:true, defaultWardrobe:"School uniform, barefoot" },
      ],
      scenes: [{ id:"sr1", number:1, title:"First Lesson", location:"Ustad's music room", time:"00:00–01:20", camera:{shotType:"Close Up",movement:"Static — Locked Off",lighting:"Golden Hour",atmosphere:"Intimate"}, chars:{cr1:{emotion:"Doubt",wardrobeOverride:null,mannerismOverride:null,dialogue:"Music cannot be taught to someone who cannot hear it.",deliveryEmo:"Melancholy",blocking:"Seated with sitar, facing window",continuityNotes:""},cr2:{emotion:"Determination",wardrobeOverride:null,mannerismOverride:null,dialogue:"",deliveryEmo:"Determination",blocking:"Kneeling, hands pressed to floor",continuityNotes:""}}, validated:true, issues:[], status:"locked", storyboard:{panels:[],version:0,generating:false}, takes:[], screenplay:{heading:"INT.",slugline:"USTAD'S MUSIC ROOM — LATE AFTERNOON",action:"Dust motes drift through shafts of golden light. USTAD FARID sits cross-legged, his sitar across his lap. MEERA kneels across from him, palms pressed flat against the wooden floor.",dialogueBlocks:[{charId:"cr1",line:"Music cannot be taught to someone who cannot hear it.",parenthetical:"not unkindly"}],transition:"DISSOLVE TO:",beat:"Setup",notes:"Establish the central tension — can music transcend hearing?",duration:80} }],
      screenplay: { title:"The Last Raga", logline:"An aging sitar master takes on one final student — a deaf girl who can feel music through vibrations.", synopsis:"", acts:[{id:"ar1",name:"Act I — Doubt",scenes:["sr1"]}] },
    },
    "proj_neon": {
      bible: { genre: "Fantasy", era: "Near Future", colorGrade: "Cyberpunk Neon", cameraBody: "Sony Venice 2", lensType: "Anamorphic", filmStock: "Digital ACES", aspectRatio: "2.39:1 Anamorphic", focalLength: "24mm Wide" },
      chars: [
        { id:"cn1", name:"Zara", color:T.PINK, physical:{Hair:"Bright pink streaks","Skin Tone":"Medium brown",Build:"Wiry",Height:"5'3\"","Scars/Marks":"Neon tattoo on neck","Facial Hair":"—","Age Range":"Early 20s","Distinguishing Feature":"AR cooking goggles on forehead"}, mannerisms:["Flips knife while talking","Tastes everything","Talks to her food","Dances while cooking"], defaultEmotion:"Joy", loraRef:false, defaultWardrobe:"Neon-trimmed apron, cargo pants, platform boots" },
      ],
      scenes: [{ id:"sn1", number:1, title:"Grandmother's Recipe", location:"Neon Bazaar night market", time:"00:00–00:35", camera:{shotType:"Wide Shot",movement:"Dolly — Push In",lighting:"Neon Noir",atmosphere:"Electric"}, chars:{cn1:{emotion:"Nostalgia",wardrobeOverride:null,mannerismOverride:null,dialogue:"Nani always said the secret ingredient was memory.",deliveryEmo:"Wistful",blocking:"Standing at street food cart, holographic menu floating",continuityNotes:""}}, validated:true, issues:[], status:"draft", storyboard:{panels:[],version:0,generating:false}, takes:[], screenplay:{heading:"EXT.",slugline:"NEON BAZAAR NIGHT MARKET — NIGHT",action:"Holographic signs flicker above a maze of food stalls. ZARA stands behind her cart, AR goggles pushed up, watching a recipe projected from her grandmother's old data chip.",dialogueBlocks:[{charId:"cn1",line:"Nani always said the secret ingredient was memory.",parenthetical:"touching the data chip"}],transition:"CUT TO:",beat:"Setup",notes:"Establish Zara's world — future Mumbai where food and data intersect.",duration:35} }],
      screenplay: { title:"Neon Bazaar", logline:"A street food vendor discovers her grandmother's recipes unlock memories stored in Mumbai's quantum network.", synopsis:"", acts:[{id:"an1",name:"Act I — Discovery",scenes:["sn1"]}] },
    },
  });
  ...
  const saveCurrentProjectData = () => {
    if (activeProjectId) {
      projectDataStore.current[activeProjectId] = {
        bible: JSON.parse(JSON.stringify(bible)),
  ...
  // Load project data into live state
  const loadProjectData = (projId) => {
    const data = projectDataStore.current[projId];
    if (!data || data === "LIVE") return; // LIVE sentinel means data is already in state
    setBible(data.bible || initBible());
    setChars(data.chars || initChars());
    setScenes(data.scenes || initScenes());
    setScreenplay(data.screenplay || initScreenplay());
    setActiveSceneId(data.scenes?.[0]?.id || "s1");
    setSpEditScene(data.scenes?.[0]?.id || "s1");
  };

  // ─── PROJECT PORTFOLIO STATE ───
  const [projects, setProjects] = useState(PROJECTS_SEED);
  const [activeProjectId, setActiveProjectId] = useState("proj_shadows");
  const [dashSearch, setDashSearch] = useState("");
  const [dashFilter, setDashFilter] = useState("all"); // all | active | completed | archived
  const [showNewProject, setShowNewProject] = useState(false);
  const [newProjectTitle, setNewProjectTitle] = useState("");
  const [newProjectLogline, setNewProjectLogline] = useState("");
  const [newProjectTemplate, setNewProjectTemplate] = useState("tpl_short");

  // ─── AUTH & TEAM STATE ───
  const [currentUserId, setCurrentUserId] = useState("u1"); // simulated login
  const [team, setTeam] = useState(TEAM_SEED);
  const [teamMembers] = useState(USERS_SEED);
  const [showUserMenu, setShowUserMenu] = useState(false);
  const currentUser = teamMembers.find(u => u.id === currentUserId) || teamMembers[0];
  const currentRole = ROLES[currentUser.role] || ROLES.auditor;
  const can = currentRole.can || {};
  // Permission helpers for clean reads in JSX
  const canEditScreenplay = can.screenplay === "edit";
  const canViewScreenplay = can.screenplay === "edit" || can.screenplay === "view";
  const canEditConsole = can.console === "edit";
  const canViewConsole = can.console !== "none";
  const canEditStoryboard = can.storyboard === "edit";
  const canApproveStoryboard = can.storyboard === "edit" || can.storyboard === "approve";
  const canGenerate = can.generate === true;
  const canReviewFull = can.review === "full";
  const canReviewAnnotate = can.review === "full" || can.review === "annotate";
  const canEditAudio = can.audio === "edit";
  const canViewAudio = can.audio !== "none";
  const canEditTimeline = can.timeline === "edit";
  const canEditTeam = can.team === "edit";

  // ─── NOTIFICATION FEED — Build 2.4 ───
  const [appNotifs, setAppNotifs] = useState([
    { id: "n1", type: "status", message: "Priya advanced Scene 1 to Reviewed", userId: "u2", timestamp: Date.now() - 7200000, read: true },
    { id: "n2", type: "comment", message: "Arjun commented on Scene 1: \"Budget approved for generation\"", userId: "u3", timestamp: Date.now() - 3600000, read: false },
    { id: "n3", type: "approval", message: "Nadia approved Take 1 for Scene 1", userId: "u4", timestamp: Date.now() - 1800000, read: false },
  ]);
  const [showNotifPanel, setShowNotifPanel] = useState(false);
  const unreadCount = appNotifs.filter(n => !n.read).length;

  const pushNotif = (type, message) => {
    setAppNotifs(prev => [{ id: `n_${Date.now()}`, type, message, userId: currentUserId, timestamp: Date.now(), read: false }, ...prev].slice(0, 50));
  };

  // ─── SCENE COMMENTS — Build 2.4 ───
  const [sceneComments, setSceneComments] = useState([
    { id: "sc1", sceneId: "s1", userId: "u3", text: "Budget approved for this scene. Generation can proceed.", timestamp: Date.now() - 3600000, resolved: false },
    { id: "sc2", sceneId: "s1", userId: "u2", text: "Dialogue in block 2 needs one more pass — Arjun's line feels expository. @Rishi thoughts?", timestamp: Date.now() - 5400000, resolved: false },
    { id: "sc3", sceneId: "s2", userId: "u4", text: "The confrontation pacing looks good from a client perspective. No notes.", timestamp: Date.now() - 1200000, resolved: true },
  ]);
  const [commentInput, setCommentInput] = useState("");

  const addComment = (sceneId, text) => {
    if (!text.trim()) return;
    const comment = { id: `sc_${Date.now()}`, sceneId, userId: currentUserId, text: text.trim(), timestamp: Date.now(), resolved: false };
    setSceneComments(prev => [...prev, comment]);
    pushNotif("comment", `${currentUser.name} commented on Scene ${scenes.find(s=>s.id===sceneId)?.number||"?"}: "${text.trim().slice(0,40)}${text.length>40?"…":""}"`);
    setCommentInput("");
  };

  const resolveComment = (commentId) => {
    setSceneComments(prev => prev.map(c => c.id === commentId ? { ...c, resolved: !c.resolved } : c));
  };

  // ─── AUDIT TRAIL — Build 2.4 ───
  const [auditLog, setAuditLog] = useState([
    { id: "a1", sceneId: "s1", userId: "u1", action: "status_change", from: "draft", to: "generated", timestamp: Date.now() - 86400000, note: "" },
    { id: "a2", sceneId: "s2", userId: "u2", action: "status_change", from: "draft", to: "reviewed", timestamp: Date.now() - 43200000, note: "Script review complete" },
  ]);

  const logAudit = (sceneId, action, details = {}) => {
    setAuditLog(prev => [...prev, { id: `a_${Date.now()}`, sceneId, userId: currentUserId, action, timestamp: Date.now(), ...details }]);
  };

  // ─── VERSION CONTROL — Build 2.5 ───
  const [versionHistory, setVersionHistory] = useState([
    { id: "v1", sceneId: "s1", userId: "u1", timestamp: Date.now() - 172800000, label: "Initial screenplay draft",
      snapshot: { action: "Steam curls through amber platform lights...", dialogueCount: 2, status: "draft", shotType: "Wide Shot" }},
    { id: "v2", sceneId: "s1", userId: "u2", timestamp: Date.now() - 86400000, label: "Dialogue polish — Meera's opening line revised",
      snapshot: { action: "Steam curls through amber platform lights. A sea of commuters...", dialogueCount: 2, status: "reviewed", shotType: "Wide Shot" }},
    { id: "v3", sceneId: "s1", userId: "u1", timestamp: Date.now() - 43200000, label: "Camera change: Steadicam, Neon Noir lighting",
      snapshot: { action: "Steam curls through amber platform lights. A sea of commuters...", dialogueCount: 2, status: "generated", shotType: "Wide Shot" }},
  ]);
  const [showVersionPanel, setShowVersionPanel] = useState(false);
  const [selectedVersion, setSelectedVersion] = useState(null); // version id for diff view
  const [showVersionDiff, setShowVersionDiff] = useState(false);

  // Snapshot current scene state as a version
  const createVersion = (sceneId, label) => {
    ...
    const ver = {
      ...
      timestamp: Date.now(),
      label: label || `Snapshot by ${currentUser.name}`,
      snapshot: {
        action: scene.screenplay?.action?.slice(0, 200) || "",
        dialogueCount: scene.screenplay?.dialogueBlocks?.length || 0,
        status: scene.status,
        shotType: scene.camera?.shotType || "—",
        charCount: Object.keys(scene.chars).length,
        panelCount: scene.storyboard?.panels?.length || 0,
        takeCount: scene.takes?.length || 0,
      },
      fullData: JSON.parse(JSON.stringify(scene)),
    };
    setVersionHistory(prev => [ver, ...prev]);
    pushNotif("status", `${currentUser.name} saved version: "${ver.label}" for Scene ${scene.number}`);
    logAudit(sceneId, "version_saved", { label: ver.label });
    notify("success", `Version saved: "${ver.label}"`);
  };

  // Restore a scene from a version snapshot
  const restoreVersion = (versionId) => {
    const ver = versionHistory.find(v => v.id === versionId);
    if (!ver?.fullData) { notify("error", "Full snapshot data not available for this version."); return; }
    setScenes(prev => prev.map(s => s.id === ver.sceneId ? { ...ver.fullData, id: s.id, number: s.number } : s));
    pushNotif("status", `${currentUser.name} restored Scene ${scenes.find(s=>s.id===ver.sceneId)?.number||"?"} to version "${ver.label}"`);
    logAudit(ver.sceneId, "version_restored", { label: ver.label, versionId });
    notify("success", `Restored to "${ver.label}"`);
    setShowVersionDiff(false);
  };

  // ─── PERSISTENCE: Boot ───
  // Auto-load disabled in multi-project mode — projectDataStore handles in-session isolation.
  // Use Export/Import for cross-session persistence.
  useEffect(() => {
    (async () => {
      try { await window.storage.delete("crushed-project"); } catch(e) {} // clear any stale single-project data
      setTimeout(() => setOk(true), 60);
    })();
  }, []);

  // ─── PERSISTENCE: Auto-save disabled in multi-project mode ───
  // In-session state is managed by projectDataStore. Use Export/Import for cross-session.
  ...
  const exportProject = () => {
    const data = JSON.stringify({
      ...
      _meta: { exportedAt: new Date().toISOString(), version: "6.1", title: screenplay.title },
    }, null, 2);
  ...
  const [view, setView] = useState("dashboard"); // dashboard | screenplay | console | characters | continuity | review | timeline
  ...
  const openProject = (projId) => {
    if (projId === activeProjectId) { setView("screenplay"); return; }
    ...
    setProjects(prev => prev.map(p => p.id === projId ? { ...p, updatedAt: new Date().toISOString() } : p));
    setView("screenplay");
    notify("success", `Opened "${projects.find(p => p.id === projId)?.title || "project"}"`);
  };

  const createProject = () => {
    if (!newProjectTitle.trim()) return;
    const tpl = PROJECT_TEMPLATES.find(t => t.id === newProjectTemplate) || PROJECT_TEMPLATES[1];
    const id = `proj_${Date.now()}`;
    const proj = {
      id, title: newProjectTitle.trim(), logline: newProjectLogline.trim() || "—",
      genre: "—", status: "active", createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(),
      scenesTotal: tpl.scenes, scenesLocked: 0, scenesGenerated: 0, thumbnail: tpl.icon,
      owner: currentUserId, collaborators: [currentUserId], creditUsed: 0,
    };
    // Build fresh project data from template
    const freshScenes = Array.from({length: tpl.scenes}, (_, i) => ({
      id: `s_${id}_${i+1}`, number: i+1, title: `Scene ${i+1}`, location: "", time: "",
      camera: { shotType: "Wide Shot", movement: "", lighting: "", atmosphere: "" },
      chars: {}, validated: true, issues: [], status: "draft",
      storyboard: { panels: [], version: 0, generating: false }, takes: [],
      screenplay: { heading: "INT.", slugline: "", action: "", dialogueBlocks: [], transition: "CUT TO:", beat: "", notes: "", duration: 30 },
    }));
    const freshActs = Array.from({length: tpl.acts}, (_, ai) => {
      const perAct = Math.ceil(tpl.scenes / tpl.acts);
      const start = ai * perAct;
      return { id: `act_${id}_${ai+1}`, name: `Act ${ai+1}`, scenes: freshScenes.slice(start, start + perAct).map(s=>s.id) };
    });
    const freshData = {
      bible: { genre: "—", era: "—", colorGrade: "—", cameraBody: "ARRI Alexa 35", lensType: "Spherical Prime", filmStock: "—", aspectRatio: "2.39:1 Anamorphic", focalLength: "35mm Standard" },
      chars: [],
      scenes: freshScenes,
      screenplay: { title: proj.title, logline: proj.logline, synopsis: "", acts: freshActs },
    };
    // Save current project before switching
    saveCurrentProjectData();
    clearGeneration();
    // Store new project data
    projectDataStore.current[id] = freshData;
    ...
    loadProjectData(id);
    setView("screenplay");
    notify("success", `Created "${proj.title}" from ${tpl.name} template`);
  };

  const duplicateProject = (projId) => {
    ...
    // Save current project to ensure source data is fresh
    saveCurrentProjectData();
    // Deep-clone source project data
    const srcData = projectDataStore.current[projId];
    if (srcData) {
      const cloned = JSON.parse(JSON.stringify(srcData));
      // Reset generation state in cloned data
      cloned.scenes.forEach(s => { s.status = "draft"; s.takes = []; s.audio = undefined; s.storyboard = { panels: [], version: 0, generating: false }; });
      projectDataStore.current[newId] = cloned;
    }
    setProjects(prev => [dupe, ...prev]);
    notify("success", `Duplicated "${src.title}"`);
  };

  const archiveProject = (projId) => {
    setProjects(prev => prev.map(p => p.id === projId ? { ...p, status: p.status === "archived" ? "active" : "archived", updatedAt: new Date().toISOString() } : p));
    const proj = projects.find(p => p.id === projId);
    notify("success", `${proj?.status === "archived" ? "Unarchived" : "Archived"} "${proj?.title}"`);
  };
  ...
  const deleteProject = (projId) => {
    ...
    if (!proj) return;
    // Clean up data store
    delete projectDataStore.current[projId];
    setProjects(prev => prev.filter(p => p.id !== projId));
    if (activeProjectId === projId) {
      setActiveProjectId(null);
      setView("dashboard");
    }
    notify("success", `Deleted "${proj.title}"`);
  };

  const activeProject = projects.find(p => p.id === activeProjectId);

  // ─── CREDIT MANAGEMENT ───
  const deductCredits = (cost, actionLabel) => {
    if (team.creditBalance < cost) {
      notify("error", `Insufficient credits — ${cost} required, ${team.creditBalance} remaining.`);
      return false;
    }
    setTeam(prev => ({ ...prev, creditBalance: prev.creditBalance - cost }));
    setProjects(prev => prev.map(p => p.id === activeProjectId ? { ...p, creditUsed: (p.creditUsed || 0) + cost } : p));
    pushNotif("generation", `${actionLabel}: ${cost} credits deducted (${team.creditBalance - cost} remaining)`);
    return true;
  };
  ...
  const updateSceneStatus = (sceneId, newStatus) => {
    ...
    // Approval gate: Reviewed → Locked requires Producer or Director
    if (newStatus === "locked" && oldStatus === "reviewed") {
      const canLock = can.statusGate === "all" || can.statusGate === "locked" || can.statusGate === "generated";
      if (!canLock) {
        pushNotif("approval", `${currentUser.name} requested lock approval for Scene ${scene.number}`);
        notify("success", "Lock request sent — a Producer or Director must approve.");
        logAudit(sceneId, "lock_requested", { from: oldStatus, to: newStatus });
        return;
      }
    }

    setScenes(scenes.map(s => s.id === sceneId ? { ...s, status: newStatus } : s));
    logAudit(sceneId, "status_change", { from: oldStatus, to: newStatus });
    pushNotif("status", `${currentUser.name} moved Scene ${scene.number} from ${oldStatus} to ${newStatus}`);
  };
  ...
  const generateStoryboard = (sceneId) => {
    ...
    if (!deductCredits(CREDIT_COSTS.generateStoryboard, `Scene ${scene.number} storyboard generation`)) return;
  ...
  const generateVideo = (sceneId) => {
    ...
    if (!scene) return;
    // Credit check
    if (!deductCredits(CREDIT_COSTS.generateVideo, `Scene ${scene.number} video generation`)) return;
  ...
  return (
    <div style={{minHeight:"100vh",maxWidth:"100vw",overflowX:"hidden",background:T.DARK,color:T.TXT,fontFamily:B}}>
      ...
      <style>{`
        html, body { background: ${T.DARK}; margin: 0; padding: 0; overflow-x: hidden; }
      ...
      <header className="cs-header" style={{display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:`1px solid ${T.BORDER}`,backdropFilter:"blur(20px)",position:"sticky",top:0,zIndex:100,background:`${T.DARK}dd`}}>
        <div style={{display:"flex",alignItems:"center",gap:10,flexShrink:0,minWidth:0}}>
          <div onClick={()=>{saveCurrentProjectData();setView("dashboard");}} style={{width:28,height:28,borderRadius:6,background:ACCENT,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,fontSize:12,color:"#000",fontFamily:D,cursor:"pointer",flexShrink:0}} title="Back to Dashboard">C</div>
          {view !== "dashboard" && (
            <button onClick={()=>{saveCurrentProjectData();setView("dashboard");}} style={{background:`${T.GOLD}08`,border:`1px solid ${T.GOLD}20`,borderRadius:6,padding:"5px 10px",fontFamily:M,fontSize:9,color:T.GOLD,cursor:"pointer",display:"flex",alignItems:"center",gap:4}}>
              ← Projects
            </button>
          )}
          <div>
            <div style={{display:"flex",alignItems:"center",gap:6}}>
              <span style={{fontFamily:D,fontWeight:700,fontSize:14,color:"#F0F0F5"}}>CRUSHED STUDIOS</span>
              {view !== "dashboard" && activeProject && <>
                <span style={{color:"#333",fontSize:10}}>/</span>
                <span style={{fontFamily:D,fontWeight:600,fontSize:12,color:T.GOLD}}>{activeProject.title}</span>
              </>}
            </div>
            <div style={{fontFamily:M,fontSize:8,color:"#444",letterSpacing:"0.12em"}}>{view === "dashboard" ? "PROJECT DASHBOARD · v6.1 · LAYER 2 ENTERPRISE" : "PRODUCTION CONSOLE · v6.1 · LAYER 2 ENTERPRISE PIPELINE"}</div>
          </div>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:1,minWidth:0,flexWrap:"wrap",justifyContent:"flex-end"}}>
          {view !== "dashboard" && <>
          ...
          <div style={{display:"flex",alignItems:"center",gap:-4}}>
            {teamMembers.map(u => (
              <div key={u.id} style={{
                ...
                background:`${u.color}20`,border:`1.5px solid ${u.id===currentUserId?u.color:u.online?u.color+"50":"#333"}`,
                display:"flex",alignItems:"center",justifyContent:"center",
                fontFamily:D,fontWeight:700,fontSize:9,color:u.id===currentUserId?u.color:u.online?u.color+"80":"#444",
                marginLeft:u.id===teamMembers[0].id?0:-6,position:"relative",zIndex:u.id===currentUserId?2:1,
                cursor:"default",
              }} title={`${u.name} · ${ROLES[u.role]?.label} · ${u.online?"Online":"Offline"}`}>
                {u.avatar}
                {u.online && <div style={{position:"absolute",bottom:-1,right:-1,width:6,height:6,borderRadius:"50%",background:T.GREEN,border:`1.5px solid ${T.DARK}`}}/>}
              </div>
            ))}
          </div>
          {/* Credits */}
          <div style={{display:"flex",alignItems:"center",gap:4,background:`${T.GOLD}06`,border:`1px solid ${T.GOLD}15`,borderRadius:5,padding:"3px 9px"}}>
            <span style={{fontFamily:M,fontSize:8.5,color:T.GOLD,fontWeight:600}}>{team.creditBalance.toLocaleString()}</span>
            <span style={{fontFamily:M,fontSize:7.5,color:"#555"}}>credits</span>
          </div>
          {/* Session indicator */}
          <div style={{display:"flex",alignItems:"center",gap:4}}>
            <div style={{width:5,height:5,borderRadius:"50%",background:T.GREEN,boxShadow:`0 0 6px ${T.GREEN}40`}}/>
            <span style={{fontFamily:M,fontSize:8,color:"#555"}}>In-session</span>
          </div>
          {/* Notification bell */}
          <div style={{position:"relative"}}>
            <button onClick={()=>{setShowNotifPanel(!showNotifPanel);setShowProjectMenu(false);setShowUserMenu(false);if(!showNotifPanel)setAppNotifs(prev=>prev.map(n=>({...n,read:true})));}}
              style={{background:showNotifPanel?`${T.GOLD}08`:T.INPUT,border:`1px solid ${showNotifPanel?T.GOLD+"30":T.IBORDER}`,borderRadius:5,padding:"4px 8px",fontFamily:M,fontSize:13,color:"#666",cursor:"pointer",position:"relative"}}>
              🔔
              {unreadCount > 0 && <div style={{position:"absolute",top:-3,right:-3,width:14,height:14,borderRadius:"50%",background:T.RED,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:M,fontSize:7,color:"#fff",fontWeight:700,border:`1.5px solid ${T.DARK}`}}>{unreadCount}</div>}
            </button>
            {showNotifPanel && (
              <div style={{position:"absolute",top:"100%",right:0,marginTop:4,background:T.CARD,border:`1px solid ${T.BORDER}`,borderRadius:10,padding:0,zIndex:200,width:320,maxHeight:400,boxShadow:"0 12px 48px #00000070",overflow:"hidden"}}
                ...
                <div style={{padding:"12px 14px 8px",borderBottom:`1px solid ${T.BORDER}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <span style={{fontFamily:D,fontWeight:700,fontSize:13,color:T.TXT}}>Notifications</span>
                  <button onClick={()=>setAppNotifs(prev=>prev.map(n=>({...n,read:true})))} style={{background:"transparent",border:"none",fontFamily:M,fontSize:8,color:"#555",cursor:"pointer"}}>Mark all read</button>
                </div>
                <div style={{maxHeight:340,overflowY:"auto"}}>
                  {appNotifs.length === 0 && <div style={{padding:20,textAlign:"center",fontFamily:B,fontSize:11,color:"#444"}}>No notifications</div>}
                  {appNotifs.map(n => {
                    const author = teamMembers.find(u=>u.id===n.userId);
                    const typeIcon = n.type==="status"?"📋":n.type==="comment"?"💬":n.type==="approval"?"✅":n.type==="generation"?"⚡":"📌";
                    ...
                    return (
                      <div key={n.id} style={{padding:"10px 14px",borderBottom:`1px solid ${T.BORDER}`,background:n.read?"transparent":`${T.GOLD}04`,display:"flex",gap:8,alignItems:"flex-start"}}>
                        <div style={{width:22,height:22,borderRadius:5,background:`${author?.color||T.GOLD}15`,border:`1px solid ${author?.color||T.GOLD}30`,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:D,fontSize:8,color:author?.color||T.GOLD,flexShrink:0,marginTop:1}}>{author?.avatar||"?"}</div>
                        <div style={{flex:1}}>
                          <div style={{fontFamily:B,fontSize:10.5,color:n.read?"#777":"#ccc",lineHeight:1.5}}>{typeIcon} {n.message}</div>
                          <div style={{fontFamily:M,fontSize:7.5,color:"#444",marginTop:2}}>{ageStr}</div>
                        </div>
                        {!n.read && <div style={{width:6,height:6,borderRadius:"50%",background:T.GOLD,flexShrink:0,marginTop:6}}/>}
                      </div>
                    );
                  })}
                </div>
          ...
          {/* Project menu */}
          <div style={{position:"relative"}}>
            <button onClick={()=>{setShowProjectMenu(!showProjectMenu);setShowUserMenu(false);}} style={{background:T.INPUT,border:`1px solid ${T.IBORDER}`,borderRadius:5,padding:"4px 10px",fontFamily:M,fontSize:9,color:"#666",cursor:"pointer",display:"flex",alignItems:"center",gap:4}}>
          ...
          {/* User avatar + menu */}
          <div style={{position:"relative"}}>
            <button onClick={()=>{setShowUserMenu(!showUserMenu);setShowProjectMenu(false);}} style={{
              display:"flex",alignItems:"center",gap:8,padding:"4px 10px 4px 4px",
              background:showUserMenu?`${currentRole.color}08`:T.INPUT,
              border:`1px solid ${showUserMenu?currentRole.color+"30":T.IBORDER}`,
              borderRadius:8,cursor:"pointer",
            }}>
              <div style={{width:26,height:26,borderRadius:6,background:`${currentUser.color}20`,border:`1.5px solid ${currentUser.color}`,
                display:"flex",alignItems:"center",justifyContent:"center",fontFamily:D,fontWeight:700,fontSize:11,color:currentUser.color}}>
                {currentUser.avatar}
              </div>
              <div style={{textAlign:"left"}}>
                <div style={{fontFamily:D,fontWeight:600,fontSize:10,color:T.TXT,lineHeight:1.2}}>{currentUser.name.split(" ")[0]}</div>
                <div style={{fontFamily:M,fontSize:7,color:currentRole.color}}>{currentRole.icon} {currentRole.label}</div>
              </div>
              <span style={{fontFamily:M,fontSize:7,color:"#444"}}>▾</span>
            </button>
            {showUserMenu && (
              <div style={{position:"absolute",top:"100%",right:0,marginTop:4,background:T.CARD,border:`1px solid ${T.BORDER}`,borderRadius:10,padding:6,zIndex:200,minWidth:240,boxShadow:"0 12px 48px #00000070"}}
                onClick={()=>setShowUserMenu(false)}>
                {/* Team name + plan */}
                <div style={{padding:"8px 12px 6px",borderBottom:`1px solid ${T.BORDER}`,marginBottom:4}}>
                  <div style={{fontFamily:D,fontWeight:700,fontSize:12,color:T.TXT}}>{team.name}</div>
                  <div style={{fontFamily:M,fontSize:8,color:T.GOLD,letterSpacing:"0.06em",textTransform:"uppercase",marginTop:2}}>{team.plan} plan · {team.creditBalance.toLocaleString()} credits</div>
                </div>
                {/* Switch user */}
                <div style={{padding:"4px 8px 2px"}}>
                  <div style={{fontFamily:M,fontSize:7.5,color:"#444",letterSpacing:"0.06em",textTransform:"uppercase",marginBottom:4}}>Switch User (Demo)</div>
                </div>
                {teamMembers.map(u => {
                  ...
                  return (
                    <button key={u.id} onClick={()=>{
                      ...
                      // Auto-redirect to first permitted view for this role
                      const r = ROLES[u.role]?.can || {};
                      const firstView = r.screenplay !== "none" ? "screenplay" : r.console !== "none" ? "console" : r.review !== "none" ? "review" : "review";
                      setView(firstView);
                      notify("success",`Switched to ${u.name} (${role?.label})`);
                    }} style={{
                      display:"flex",alignItems:"center",gap:10,width:"100%",padding:"8px 12px",
                      background:isCurrent?`${u.color}08`:"transparent",border:"none",borderRadius:6,cursor:"pointer",textAlign:"left",
                    }} onMouseEnter={e=>{if(!isCurrent)e.currentTarget.style.background=`${u.color}06`;}} onMouseLeave={e=>{if(!isCurrent)e.currentTarget.style.background="transparent";}}>
                      <div style={{width:24,height:24,borderRadius:5,background:`${u.color}20`,border:`1.5px solid ${isCurrent?u.color:u.color+"40"}`,
                        display:"flex",alignItems:"center",justifyContent:"center",fontFamily:D,fontWeight:700,fontSize:10,color:u.color,flexShrink:0,position:"relative"}}>
                        {u.avatar}
                        {u.online && <div style={{position:"absolute",bottom:-2,right:-2,width:5,height:5,borderRadius:"50%",background:T.GREEN,border:`1px solid ${T.DARK}`}}/>}
                      </div>
                      <div style={{flex:1}}>
                        <div style={{fontFamily:D,fontWeight:600,fontSize:10.5,color:isCurrent?T.TXT:"#888"}}>{u.name}{isCurrent?" (you)":""}</div>
                        <div style={{fontFamily:M,fontSize:7.5,color:role?.color||"#555"}}>{role?.icon} {role?.label} · {u.online?"Online":"Offline"}</div>
                      </div>
                      {isCurrent && <span style={{fontFamily:M,fontSize:8,color:T.GREEN}}>✓</span>}
                    </button>
                  );
                })}
                {/* Role legend */}
                <div style={{borderTop:`1px solid ${T.BORDER}`,marginTop:4,padding:"8px 12px 4px"}}>
                  <div style={{fontFamily:M,fontSize:7.5,color:"#444",letterSpacing:"0.06em",textTransform:"uppercase",marginBottom:4}}>Roles</div>
                  <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
                    {Object.entries(ROLES).map(([k,r]) => (
                      <span key={k} style={{fontFamily:M,fontSize:7,color:r.color,background:`${r.color}08`,border:`1px solid ${r.color}15`,borderRadius:3,padding:"2px 6px"}}>{r.icon} {r.label}</span>
                    ))}
                  </div>
                </div>
              </div>
      ...
      <main className="cs-main" style={{opacity:ok?1:0,transform:ok?"none":"translateY(10px)",transition:"all 0.6s"}}>
        ...
        {/* ═══ ROLE ACCESS BANNER ═══ */}
        {(() => {
          const viewPerms = { screenplay: can.screenplay, console: can.console, characters: can.screenplay || can.console ? "view" : "none", continuity: can.console, review: can.review, timeline: can.timeline };
          const perm = viewPerms[view];
          if (!perm || perm === "edit" || perm === "full" || perm === true) return null;
          const isView = perm === "view" || perm === "annotate" || perm === "approve" || perm === "camera";
          if (!isView) return null;
          const modeLabel = perm === "view" ? "View Only" : perm === "annotate" ? "Annotate Only" : perm === "approve" ? "Approve Only" : perm === "camera" ? "Camera & Grade Only" : perm;
          return (
            <div style={{display:"flex",alignItems:"center",gap:8,padding:"6px 14px",marginBottom:10,background:`${currentRole.color}06`,border:`1px solid ${currentRole.color}15`,borderRadius:8}}>
              <span style={{fontFamily:M,fontSize:9,color:currentRole.color}}>{currentRole.icon} {currentRole.label}</span>
              <span style={{fontFamily:M,fontSize:8.5,color:"#555"}}>·</span>
              <span style={{fontFamily:M,fontSize:8.5,color:"#666"}}>{modeLabel}</span>
            </div>
          );
        })()}

        {/* ═══════════════════════════════════════════════════════════════
            VIEW: DASHBOARD — Project Portfolio
            ═══════════════════════════════════════════════════════════════ */}
        {view === "dashboard" && (() => {
          const filtered = projects.filter(p => {
            if (dashFilter !== "all" && p.status !== dashFilter) return false;
            if (dashSearch && !p.title.toLowerCase().includes(dashSearch.toLowerCase()) && !p.logline.toLowerCase().includes(dashSearch.toLowerCase())) return false;
            return true;
          });
          const STATUS_COLORS = { active: T.GREEN, completed: T.CYAN, archived: "#555" };
          ...
          <div>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20}}>
              <div>
                <div style={{fontFamily:D,fontWeight:700,fontSize:22,color:T.TXT,letterSpacing:"0.02em"}}>Projects</div>
                <div style={{fontFamily:B,fontSize:12,color:"#555",marginTop:4}}>{projects.filter(p=>p.status==="active").length} active · {projects.filter(p=>p.status==="completed").length} completed · {team.creditBalance.toLocaleString()} credits remaining</div>
              </div>
              <button onClick={()=>setShowNewProject(true)} style={{background:ACCENT,border:"none",borderRadius:8,padding:"10px 22px",fontFamily:D,fontWeight:700,fontSize:13,color:"#000",cursor:"pointer",boxShadow:`0 4px 20px ${T.GOLD}30`}}>
                + New Project
              </button>
            </div>
            ...
            <div style={{display:"flex",gap:8,marginBottom:16,alignItems:"center"}}>
              <input value={dashSearch} onChange={e=>setDashSearch(e.target.value)} placeholder="Search projects..."
                style={{flex:1,background:T.INPUT,border:`1px solid ${T.IBORDER}`,borderRadius:8,padding:"9px 14px",color:T.TXT,fontFamily:B,fontSize:12,outline:"none"}}/>
              {["all","active","completed","archived"].map(f => (
                <button key={f} onClick={()=>setDashFilter(f)} style={{
                  background:dashFilter===f?`${T.GOLD}10`:"transparent",border:`1px solid ${dashFilter===f?T.GOLD+"30":T.IBORDER}`,
                  borderRadius:6,padding:"7px 14px",fontFamily:M,fontSize:9,color:dashFilter===f?T.GOLD:"#555",cursor:"pointer",textTransform:"capitalize",
                }}>{f}</button>
              ))}
            </div>
            ...
            <div className="cs-g2" style={{gap:12}}>
              {filtered.map(proj => {
                const statusColor = STATUS_COLORS[proj.status] || "#555";
                const progress = proj.scenesTotal > 0 ? Math.round((proj.scenesGenerated / proj.scenesTotal) * 100) : 0;
                const collabs = proj.collaborators.map(uid => teamMembers.find(u=>u.id===uid)).filter(Boolean);
                const isActive = proj.id === activeProjectId;
                return (
                  <div key={proj.id} onClick={()=>openProject(proj.id)} style={{
                    background:T.CARD,border:`1px solid ${isActive?T.GOLD+"30":T.BORDER}`,borderRadius:12,
                    padding:0,cursor:"pointer",position:"relative",overflow:"hidden",
                    boxShadow:isActive?`0 0 16px ${T.GOLD}10`:"none",transition:"all 0.2s",
                  }} onMouseEnter={e=>{if(!isActive)e.currentTarget.style.borderColor=T.GOLD+"20";}} onMouseLeave={e=>{if(!isActive)e.currentTarget.style.borderColor=T.BORDER;}}>
                    {/* Top accent */}
                    <div style={{height:3,background:statusColor,opacity:0.4}}/>

                    {/* Thumbnail area */}
                    <div style={{height:80,background:`${statusColor}04`,display:"flex",alignItems:"center",justifyContent:"center",borderBottom:`1px solid ${T.BORDER}`,position:"relative"}}>
                      <span style={{fontSize:36,opacity:0.3}}>{proj.thumbnail}</span>
                      {isActive && <div style={{position:"absolute",top:8,right:8,fontFamily:M,fontSize:7,color:T.GOLD,background:`${T.GOLD}15`,borderRadius:3,padding:"2px 6px"}}>CURRENT</div>}
                      <div style={{position:"absolute",bottom:6,right:8,fontFamily:M,fontSize:7,color:statusColor,background:`${statusColor}10`,borderRadius:3,padding:"2px 6px",textTransform:"capitalize"}}>{proj.status}</div>
                    </div>

                    {/* Content */}
                    <div style={{padding:"12px 14px"}}>
                      <div style={{fontFamily:D,fontWeight:700,fontSize:14,color:T.TXT,marginBottom:3,lineHeight:1.3}}>{proj.title}</div>
                      <div style={{fontFamily:B,fontSize:10,color:"#666",lineHeight:1.5,marginBottom:10,minHeight:30,display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical",overflow:"hidden"}}>{proj.logline}</div>

                      {/* Progress bar */}
                      <div style={{marginBottom:8}}>
                        <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
                          <span style={{fontFamily:M,fontSize:7.5,color:"#555"}}>{proj.scenesGenerated}/{proj.scenesTotal} scenes generated</span>
                          <span style={{fontFamily:M,fontSize:7.5,color:statusColor}}>{progress}%</span>
                        </div>
                      ...
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                        <div style={{display:"flex",gap:-3}}>
                          {collabs.slice(0,4).map((u,i) => (
                            <div key={u.id} style={{width:18,height:18,borderRadius:"50%",background:`${u.color}20`,border:`1px solid ${u.color}40`,
                              display:"flex",alignItems:"center",justifyContent:"center",fontFamily:D,fontSize:7,color:u.color,
                              marginLeft:i>0?-5:0,zIndex:4-i}} title={u.name}>
                              {u.avatar}
                            </div>
                          ))}
                        </div>
                        <div style={{display:"flex",alignItems:"center",gap:6}}>
                          <span style={{fontFamily:M,fontSize:7,color:"#444"}}>{proj.genre}</span>
                          <span style={{fontFamily:M,fontSize:7,color:"#333"}}>·</span>
                          <span style={{fontFamily:M,fontSize:7,color:proj.creditUsed > 0 ? T.GOLD : "#444"}}>{proj.creditUsed > 0 ? proj.creditUsed.toLocaleString()+" credits used" : "No credits used"}</span>
                        </div>
                      </div>

                      {/* Action buttons */}
                      <div style={{display:"flex",gap:4,marginTop:8,borderTop:`1px solid ${T.BORDER}`,paddingTop:8}} onClick={e=>e.stopPropagation()}>
                        <button onClick={()=>openProject(proj.id)} style={{flex:1,background:`${T.GOLD}08`,border:`1px solid ${T.GOLD}20`,borderRadius:5,padding:"5px 0",fontFamily:M,fontSize:8,color:T.GOLD,cursor:"pointer"}}>Open</button>
                        <button onClick={()=>duplicateProject(proj.id)} style={{background:T.INPUT,border:`1px solid ${T.IBORDER}`,borderRadius:5,padding:"5px 8px",fontFamily:M,fontSize:8,color:"#555",cursor:"pointer"}}>📋</button>
                        <button onClick={()=>archiveProject(proj.id)} style={{background:T.INPUT,border:`1px solid ${T.IBORDER}`,borderRadius:5,padding:"5px 8px",fontFamily:M,fontSize:8,color:proj.status==="archived"?T.GREEN:"#555",cursor:"pointer"}}>{proj.status==="archived"?"↩":"📦"}</button>
                        <button onClick={()=>setConfirmAction({title:"Delete Project",message:`Delete "${proj.title}"? This cannot be undone.`,onConfirm:()=>{deleteProject(proj.id);setConfirmAction(null);}})} style={{background:T.INPUT,border:`1px solid ${T.IBORDER}`,borderRadius:5,padding:"5px 8px",fontFamily:M,fontSize:8,color:T.RED,cursor:"pointer"}}>🗑</button>
                      </div>
                    </div>
                  </div>
                );
            ...
            {filtered.length === 0 && (
              <div style={{textAlign:"center",padding:"40px 20px"}}>
                ...
                <div style={{fontFamily:D,fontWeight:700,fontSize:14,color:"#555",marginBottom:6}}>No Projects Found</div>
                <div style={{fontFamily:B,fontSize:11,color:"#444"}}>{dashSearch ? "Try a different search term." : "Create your first project to get started."}</div>
              </div>
            )}
            ...
            {showNewProject && (
              <div style={{position:"fixed",inset:0,background:"#000C",backdropFilter:"blur(6px)",zIndex:9990,display:"flex",alignItems:"center",justifyContent:"center",padding:24}}
                ...
                <div onClick={e=>e.stopPropagation()} style={{background:T.CARD,border:`1px solid ${T.BORDER}`,borderRadius:14,padding:28,maxWidth:480,width:"100%",boxShadow:"0 20px 80px #00000080"}}>
                  <div style={{fontFamily:D,fontWeight:700,fontSize:18,color:T.TXT,marginBottom:4}}>New Project</div>
                  <div style={{fontFamily:B,fontSize:11,color:"#555",marginBottom:20}}>Choose a template and give your project a name.</div>

                  {/* Template selection */}
                  <div style={{fontFamily:M,fontSize:8,color:"#444",letterSpacing:"0.06em",textTransform:"uppercase",marginBottom:8}}>Template</div>
                  <div style={{display:"flex",gap:6,marginBottom:16,flexWrap:"wrap"}}>
                    {PROJECT_TEMPLATES.map(tpl => (
                      <button key={tpl.id} onClick={()=>setNewProjectTemplate(tpl.id)} style={{
                        background:newProjectTemplate===tpl.id?`${T.GOLD}10`:"transparent",
                        border:`1px solid ${newProjectTemplate===tpl.id?T.GOLD+"35":T.IBORDER}`,
                        borderRadius:8,padding:"10px 14px",cursor:"pointer",textAlign:"left",minWidth:120,flex:1,
                      }}>
                        <div style={{fontSize:16,marginBottom:4}}>{tpl.icon}</div>
                        <div style={{fontFamily:D,fontWeight:600,fontSize:11,color:newProjectTemplate===tpl.id?T.GOLD:"#888"}}>{tpl.name}</div>
                        <div style={{fontFamily:M,fontSize:7.5,color:"#555",marginTop:2}}>{tpl.scenes} scene{tpl.scenes!==1?"s":""} · {tpl.acts} act{tpl.acts!==1?"s":""}</div>
                      </button>
                    ))}
                  </div>

                  {/* Title + logline */}
                  <div style={{fontFamily:M,fontSize:8,color:"#444",letterSpacing:"0.06em",textTransform:"uppercase",marginBottom:6}}>Title</div>
                  <input value={newProjectTitle} onChange={e=>setNewProjectTitle(e.target.value)} placeholder="Project title..."
                    style={{width:"100%",boxSizing:"border-box",background:T.INPUT,border:`1px solid ${T.IBORDER}`,borderRadius:8,padding:"10px 14px",color:T.TXT,fontFamily:D,fontWeight:600,fontSize:14,outline:"none",marginBottom:12}}/>

                  <div style={{fontFamily:M,fontSize:8,color:"#444",letterSpacing:"0.06em",textTransform:"uppercase",marginBottom:6}}>Logline (optional)</div>
                  <textarea value={newProjectLogline} onChange={e=>setNewProjectLogline(e.target.value)} placeholder="One-sentence story summary..." rows={2}
                    style={{width:"100%",boxSizing:"border-box",background:T.INPUT,border:`1px solid ${T.IBORDER}`,borderRadius:8,padding:"10px 14px",color:"#999",fontFamily:B,fontSize:11,outline:"none",resize:"vertical",marginBottom:20}}/>

                  {/* Actions */}
                  <div style={{display:"flex",justifyContent:"flex-end",gap:8}}>
                    <button onClick={()=>setShowNewProject(false)} style={{background:T.INPUT,border:`1px solid ${T.IBORDER}`,borderRadius:8,padding:"10px 20px",fontFamily:D,fontWeight:600,fontSize:12,color:"#999",cursor:"pointer"}}>Cancel</button>
                    <button onClick={createProject} disabled={!newProjectTitle.trim()} style={{
                      background:newProjectTitle.trim()?ACCENT:"#333",border:"none",borderRadius:8,padding:"10px 24px",
                      fontFamily:D,fontWeight:700,fontSize:12,color:newProjectTitle.trim()?"#000":"#666",cursor:newProjectTitle.trim()?"pointer":"not-allowed",
                      boxShadow:newProjectTitle.trim()?`0 4px 20px ${T.GOLD}30`:"none",
        ...
        {/* ═══ PROJECT VIEWS (only when a project is open, not on dashboard) ═══ */}
        {view !== "dashboard" && (<>

        {/* ═══ VIEW TABS — Permission-gated per role ═══ */}
        <div style={{display:"flex",gap:6,marginBottom:20}}>
          {[
            {id:"screenplay",icon:"📝",label:"Screenplay",desc:"Write, structure & lock scenes",show:canViewScreenplay},
            {id:"console",icon:"🎬",label:"Scene Console",desc:"Configure & generate scenes",show:canViewConsole},
            {id:"characters",icon:"👤",label:"Character Bible",desc:"Identity cards & LoRA status",show:canViewScreenplay||canViewConsole},
            {id:"continuity",icon:"🔗",label:"Continuity Tracker",desc:"Cross-scene validation",show:canViewConsole},
            {id:"review",icon:"🎥",label:"Review Interface",desc:"Review generated output",show:canReviewAnnotate},
            {id:"timeline",icon:"🎞️",label:"Timeline / NLE",desc:"Assemble scenes into film",show:canEditTimeline},
        ...
        <div style={{marginBottom:18}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
            ...
            <div style={{display:"flex",gap:6}}>
              ...
              {canEditScreenplay && <button onClick={addScene} style={{background:`${T.GOLD}12`,border:`1px solid ${T.GOLD}30`,borderRadius:5,padding:"4px 12px",fontFamily:M,fontSize:9,color:T.GOLD,cursor:"pointer",fontWeight:600}}>+ Add Scene</button>}
            </div>
          </div>
                ...
                <div onClick={()=>{setActiveSceneId(s.id);setSpEditScene(s.id);}} style={{
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:4}}>
                      ...
                      {canEditScreenplay && <div style={{position:"relative"}}>
        ...
        {view === "screenplay" && (
          <div>
              ...
              <div style={{background:T.CARD,border:`1px solid ${T.BORDER}`,borderRadius:10,padding:16,marginBottom:14}}>
                ...
                <div style={{marginBottom:16}}>
                  ...
                  <StatusPipeline
                    ...
                    statusGate={can.statusGate || "none"}
                  />
            ...
            {/* ── Scene Comments — Build 2.4 ── */}
            {spScene && (
              <div style={{background:T.CARD,border:`1px solid ${T.BORDER}`,borderRadius:10,padding:16,marginTop:14}}>
                <SH icon="💬" title={`COMMENTS · SCENE ${spScene.number}`}
                  tip="Leave feedback, notes, and @-mentions for team members on this scene."
                  right={<span style={{fontFamily:M,fontSize:8,color:"#555"}}>{sceneComments.filter(c=>c.sceneId===spScene.id&&!c.resolved).length} open</span>}/>

                {/* Comment input */}
                <div style={{display:"flex",gap:8,marginBottom:12}}>
                  <div style={{width:24,height:24,borderRadius:5,background:`${currentUser.color}20`,border:`1px solid ${currentUser.color}40`,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:D,fontWeight:700,fontSize:9,color:currentUser.color,flexShrink:0}}>{currentUser.avatar}</div>
                  <input value={commentInput} onChange={e=>setCommentInput(e.target.value)}
                    placeholder={`Comment as ${currentUser.name}... (use @ to mention)`}
                    onKeyDown={e=>{ if (e.key==="Enter" && commentInput.trim()) addComment(spScene.id, commentInput); }}
                  ...
                  <button onClick={()=>addComment(spScene.id, commentInput)} disabled={!commentInput.trim()}
                    style={{background:commentInput.trim()?`${T.CYAN}12`:"transparent",border:`1px solid ${commentInput.trim()?T.CYAN+"30":T.IBORDER}`,borderRadius:6,padding:"8px 14px",fontFamily:M,fontSize:9,color:commentInput.trim()?T.CYAN:"#555",cursor:commentInput.trim()?"pointer":"not-allowed",fontWeight:600}}>
                    Post
                  </button>
                </div>

                {/* Comment thread */}
                {sceneComments.filter(c=>c.sceneId===spScene.id).length === 0 && (
                  <div style={{textAlign:"center",padding:"12px 0",fontFamily:B,fontSize:11,color:"#444"}}>No comments on this scene yet.</div>
                )}
                <div style={{display:"flex",flexDirection:"column",gap:4}}>
                  {sceneComments.filter(c=>c.sceneId===spScene.id).sort((a,b)=>b.timestamp-a.timestamp).map(c => {
                    const author = teamMembers.find(u=>u.id===c.userId);
                    const role = ROLES[author?.role];
                    const age = Date.now() - c.timestamp;
                    const ageStr = age < 60000 ? "just now" : age < 3600000 ? Math.floor(age/60000)+"m ago" : age < 86400000 ? Math.floor(age/3600000)+"h ago" : Math.floor(age/86400000)+"d ago";
                    // Highlight @mentions
                    const textParts = c.text.split(/(@\w+)/g);
                    return (
                      <div key={c.id} style={{display:"flex",gap:8,padding:"10px 12px",background:c.resolved?`${T.GREEN}03`:`${T.CYAN}03`,border:`1px solid ${c.resolved?T.GREEN+"10":T.CYAN+"10"}`,borderRadius:8,opacity:c.resolved?0.6:1}}>
                        <div style={{width:24,height:24,borderRadius:5,background:`${author?.color||"#555"}15`,border:`1px solid ${author?.color||"#555"}30`,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:D,fontWeight:700,fontSize:9,color:author?.color||"#555",flexShrink:0}}>{author?.avatar||"?"}</div>
                        <div style={{flex:1}}>
                          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:3}}>
                            <span style={{fontFamily:D,fontWeight:600,fontSize:10.5,color:T.TXT}}>{author?.name||"Unknown"}</span>
                            <span style={{fontFamily:M,fontSize:7,color:role?.color||"#555",background:`${role?.color||"#555"}10`,borderRadius:3,padding:"1px 5px"}}>{role?.icon} {role?.label}</span>
                            <span style={{fontFamily:M,fontSize:7.5,color:"#444"}}>{ageStr}</span>
                            {c.resolved && <span style={{fontFamily:M,fontSize:7,color:T.GREEN}}>✓ Resolved</span>}
                          </div>
                          <div style={{fontFamily:B,fontSize:11,color:c.resolved?"#666":"#999",lineHeight:1.6,textDecoration:c.resolved?"line-through":"none"}}>
                            {textParts.map((part, pi) => part.startsWith("@") ? <span key={pi} style={{color:T.GOLD,fontWeight:600}}>{part}</span> : part)}
                          </div>
                        </div>
                        <button onClick={()=>resolveComment(c.id)} style={{background:"transparent",border:`1px solid ${T.IBORDER}`,borderRadius:4,padding:"2px 8px",fontFamily:M,fontSize:7.5,color:c.resolved?T.GREEN:"#555",cursor:"pointer",flexShrink:0,alignSelf:"flex-start"}}>
            ...
            {/* ── Audit Trail — Build 2.4 ── */}
            {spScene && auditLog.filter(a=>a.sceneId===spScene.id).length > 0 && (
              <div style={{background:T.CARD,border:`1px solid ${T.BORDER}`,borderRadius:10,padding:16,marginTop:14}}>
                <SH icon="📋" title={`AUDIT TRAIL · SCENE ${spScene.number}`} tip="History of all status transitions and significant actions on this scene."/>
                <div style={{display:"flex",flexDirection:"column",gap:2}}>
                  {auditLog.filter(a=>a.sceneId===spScene.id).sort((a,b)=>b.timestamp-a.timestamp).map(a => {
                    const author = teamMembers.find(u=>u.id===a.userId);
                    ...
                    const actionLabel = a.action === "status_change" ? `${a.from} → ${a.to}` : a.action === "lock_requested" ? "Lock approval requested" : a.action;
                    const actionColor = a.action === "status_change" ? T.GREEN : a.action === "lock_requested" ? T.ORANGE : "#555";
                    return (
                      <div key={a.id} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 10px",borderRadius:5,background:`${actionColor}03`}}>
                        <div style={{width:18,height:18,borderRadius:4,background:`${author?.color||"#555"}12`,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:D,fontSize:7,color:author?.color||"#555",flexShrink:0}}>{author?.avatar||"?"}</div>
                        <span style={{fontFamily:D,fontWeight:600,fontSize:10,color:T.TXT,minWidth:70}}>{author?.name?.split(" ")[0]||"?"}</span>
                        <span style={{fontFamily:M,fontSize:9,color:actionColor,background:`${actionColor}08`,borderRadius:3,padding:"2px 7px"}}>{actionLabel}</span>
                        {a.note && <span style={{fontFamily:B,fontSize:9,color:"#666",fontStyle:"italic"}}>"{a.note}"</span>}
                        <span style={{fontFamily:M,fontSize:7.5,color:"#444",marginLeft:"auto"}}>{ageStr}</span>
                      </div>
                    );
                  })}
                </div>
            ...
            {/* ── Version History — Build 2.5 ── */}
            {spScene && (
              <div style={{background:T.CARD,border:`1px solid ${T.BORDER}`,borderRadius:10,padding:16,marginTop:14}}>
                <SH icon="🕐" title={`VERSION HISTORY · SCENE ${spScene.number}`}
                  tip="Save snapshots of this scene's state. Restore any previous version to undo changes."
                  right={
                    <div style={{display:"flex",gap:6}}>
                      <button onClick={()=>{
                        const label = `Snapshot — ${new Date().toLocaleTimeString()} by ${currentUser.name}`;
                        createVersion(spScene.id, label);
                      }} style={{background:`${T.PURPLE}12`,border:`1px solid ${T.PURPLE}30`,borderRadius:5,padding:"4px 12px",fontFamily:M,fontSize:9,color:T.PURPLE,cursor:"pointer",fontWeight:600}}>
                        💾 Save Version
                      </button>
                      <button onClick={()=>setShowVersionPanel(!showVersionPanel)} style={{background:T.INPUT,border:`1px solid ${T.IBORDER}`,borderRadius:5,padding:"4px 10px",fontFamily:M,fontSize:9,color:"#555",cursor:"pointer"}}>
                        {showVersionPanel ? "Collapse" : `${versionHistory.filter(v=>v.sceneId===spScene.id).length} versions`}
                      </button>
                    </div>
                  }
                />

                {/* Compact: latest version only */}
                {!showVersionPanel && (() => {
                  const latest = versionHistory.filter(v=>v.sceneId===spScene.id).sort((a,b)=>b.timestamp-a.timestamp)[0];
                  if (!latest) return <div style={{fontFamily:B,fontSize:11,color:"#444"}}>No versions saved yet. Click "Save Version" to create a snapshot.</div>;
                  const author = teamMembers.find(u=>u.id===latest.userId);
                  const age = Date.now() - latest.timestamp;
                  const ageStr = age < 3600000 ? Math.floor(age/60000)+"m ago" : age < 86400000 ? Math.floor(age/3600000)+"h ago" : Math.floor(age/86400000)+"d ago";
                  return (
                    <div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 10px",background:`${T.PURPLE}04`,borderRadius:6,border:`1px solid ${T.PURPLE}10`}}>
                      <div style={{width:18,height:18,borderRadius:4,background:`${author?.color||"#555"}15`,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:D,fontSize:7,color:author?.color||"#555"}}>{author?.avatar||"?"}</div>
                      <div style={{flex:1}}>
                        <div style={{fontFamily:D,fontWeight:600,fontSize:10,color:T.TXT}}>{latest.label}</div>
                        <div style={{fontFamily:M,fontSize:7.5,color:"#555"}}>{ageStr} · {latest.snapshot?.dialogueCount || 0} dialogue blocks · {latest.snapshot?.status}</div>
                      </div>
                    </div>
                  );
                })()}

                {/* Expanded: full version list with diffs */}
                {showVersionPanel && (
                  <div style={{display:"flex",flexDirection:"column",gap:4}}>
                    {versionHistory.filter(v=>v.sceneId===spScene.id).sort((a,b)=>b.timestamp-a.timestamp).map((ver, vi, arr) => {
                      const author = teamMembers.find(u=>u.id===ver.userId);
                      const role = ROLES[author?.role];
                      const age = Date.now() - ver.timestamp;
                      const ageStr = age < 3600000 ? Math.floor(age/60000)+"m ago" : age < 86400000 ? Math.floor(age/3600000)+"h ago" : Math.floor(age/86400000)+"d ago";
                      const prev = arr[vi + 1];
                      const isSelected = selectedVersion === ver.id;

                      // Compute simple diffs from snapshots
                      const diffs = [];
                      if (prev?.snapshot) {
                        if (ver.snapshot?.status !== prev.snapshot?.status) diffs.push(`Status: ${prev.snapshot.status} → ${ver.snapshot.status}`);
                        if (ver.snapshot?.shotType !== prev.snapshot?.shotType) diffs.push(`Camera: ${prev.snapshot.shotType} → ${ver.snapshot.shotType}`);
                        if (ver.snapshot?.dialogueCount !== prev.snapshot?.dialogueCount) diffs.push(`Dialogue: ${prev.snapshot.dialogueCount} → ${ver.snapshot.dialogueCount} blocks`);
                        if ((ver.snapshot?.charCount||0) !== (prev.snapshot?.charCount||0)) diffs.push(`Cast: ${prev.snapshot?.charCount||0} → ${ver.snapshot?.charCount||0}`);
                        if ((ver.snapshot?.panelCount||0) !== (prev.snapshot?.panelCount||0)) diffs.push(`Panels: ${prev.snapshot?.panelCount||0} → ${ver.snapshot?.panelCount||0}`);
                        if ((ver.snapshot?.takeCount||0) !== (prev.snapshot?.takeCount||0)) diffs.push(`Takes: ${prev.snapshot?.takeCount||0} → ${ver.snapshot?.takeCount||0}`);
                      }

                      return (
                        <div key={ver.id} onClick={()=>setSelectedVersion(isSelected ? null : ver.id)}
                          style={{padding:"10px 12px",borderRadius:8,cursor:"pointer",
                            background:isSelected?`${T.PURPLE}08`:`${T.PURPLE}03`,
                            border:`1px solid ${isSelected?T.PURPLE+"25":T.PURPLE+"10"}`,
                            transition:"all 0.15s",
                          }}>
                          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:isSelected?8:0}}>
                            {/* Version dot + connector */}
                            <div style={{display:"flex",flexDirection:"column",alignItems:"center",flexShrink:0}}>
                              <div style={{width:10,height:10,borderRadius:"50%",background:vi===0?T.PURPLE:`${T.PURPLE}40`,border:`2px solid ${vi===0?T.PURPLE:T.PURPLE+"60"}`}}/>
                              {vi < arr.length - 1 && <div style={{width:1,height:16,background:`${T.PURPLE}20`,marginTop:2}}/>}
                            </div>
                            <div style={{flex:1}}>
                              <div style={{display:"flex",alignItems:"center",gap:6}}>
                                <span style={{fontFamily:D,fontWeight:600,fontSize:10.5,color:vi===0?T.PURPLE:T.TXT}}>{ver.label}</span>
                                {vi === 0 && <span style={{fontFamily:M,fontSize:7,color:T.PURPLE,background:`${T.PURPLE}15`,borderRadius:3,padding:"1px 5px"}}>LATEST</span>}
                              </div>
                              <div style={{display:"flex",alignItems:"center",gap:4,marginTop:2}}>
                                <div style={{width:14,height:14,borderRadius:3,background:`${author?.color||"#555"}15`,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:D,fontSize:6,color:author?.color||"#555"}}>{author?.avatar||"?"}</div>
                                <span style={{fontFamily:M,fontSize:7.5,color:role?.color||"#555"}}>{author?.name?.split(" ")[0]}</span>
                                <span style={{fontFamily:M,fontSize:7,color:"#444"}}>{ageStr}</span>
                                {diffs.length > 0 && <span style={{fontFamily:M,fontSize:7,color:T.CYAN,background:`${T.CYAN}08`,borderRadius:3,padding:"1px 5px"}}>{diffs.length} change{diffs.length!==1?"s":""}</span>}
                              </div>
                            </div>
                          </div>

                          {/* Expanded: diff details + restore */}
                          {isSelected && (
                            <div style={{marginLeft:26}}>
                              {diffs.length > 0 && (
                                <div style={{marginBottom:8}}>
                                  {diffs.map((d, di) => (
                                    <div key={di} style={{display:"flex",alignItems:"center",gap:6,padding:"3px 0"}}>
                                      <span style={{fontFamily:M,fontSize:8,color:T.CYAN}}>△</span>
                                      <span style={{fontFamily:M,fontSize:9,color:"#888"}}>{d}</span>
                                    </div>
                                  ))}
                                </div>
                              )}
                              {diffs.length === 0 && !prev && (
                                <div style={{fontFamily:M,fontSize:8.5,color:"#555",marginBottom:8}}>Initial version — no previous state to compare</div>
                              )}
                              {/* Snapshot summary */}
                              <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:8}}>
                                {[
                                  {label:"Status",value:ver.snapshot?.status||"—"},
                                  {label:"Camera",value:ver.snapshot?.shotType||"—"},
                                  {label:"Dialogue",value:(ver.snapshot?.dialogueCount||0)+" blocks"},
                                  {label:"Cast",value:(ver.snapshot?.charCount||0)+" chars"},
                                  {label:"Panels",value:(ver.snapshot?.panelCount||0)+""},
                                  {label:"Takes",value:(ver.snapshot?.takeCount||0)+""},
                                ].map(s => (
                                  <span key={s.label} style={{fontFamily:M,fontSize:7.5,color:"#555",background:"#ffffff04",borderRadius:3,padding:"2px 6px"}}>{s.label}: {s.value}</span>
                                ))}
                              </div>
                              {/* Restore button */}
                              {vi > 0 && ver.fullData && (
                                <button onClick={e=>{e.stopPropagation();setConfirmAction({title:"Restore Version",message:`Restore Scene ${spScene.number} to "${ver.label}"? Current state will be overwritten.`,onConfirm:()=>{restoreVersion(ver.id);setConfirmAction(null);}});}}
                                  style={{background:`${T.ORANGE}10`,border:`1px solid ${T.ORANGE}25`,borderRadius:5,padding:"5px 14px",fontFamily:M,fontSize:9,color:T.ORANGE,cursor:"pointer",fontWeight:600}}>
                                  ↩ Restore this version
                                </button>
                              )}
                              {vi === 0 && <span style={{fontFamily:M,fontSize:8.5,color:T.PURPLE}}>This is the current version</span>}
                              {!ver.fullData && vi > 0 && <span style={{fontFamily:M,fontSize:8.5,color:"#444"}}>Snapshot only — full restore not available for seed versions</span>}
                            </div>
                          )}
                        </div>
                      );
                    })}
                    {versionHistory.filter(v=>v.sceneId===spScene.id).length === 0 && (
                      <div style={{textAlign:"center",padding:"16px 0",fontFamily:B,fontSize:11,color:"#444"}}>No versions saved for this scene yet.</div>
                    )}
                  </div>
                )}
              </div>
          ...
          <div>
                      ...
                      {activeStoryboard.panels.map((panel, pi) => {
                        ...
                        return (
                          <div key={panel.id} className="cs-filmstrip-card" style={{minWidth:200,maxWidth:200,flexShrink:0,position:"relative"}}>
                            ...
                            {/* Action row — permission-gated */}
                            <div style={{display:"flex",gap:3,marginTop:6}}>
                              {canApproveStoryboard && !panel.approved && (
                              ...
                              {canEditStoryboard && <button onClick={(e) => {e.stopPropagation(); regeneratePanel(activeSceneId, panel.id);}}
                                disabled={isRegen}
                              ...
                              {canEditStoryboard && <button onClick={(e) => {e.stopPropagation(); deletePanel(activeSceneId, panel.id);}}
            ...
            <div style={{background:T.CARD,border:`1px solid ${T.BORDER}`,borderRadius:10,padding:16,marginBottom:14}}>
              <SH n={2} icon="🎥" title="CINEMATOGRAPHY"
                right={canEditConsole ? <button onClick={surpriseMe} style={{background:`${T.GOLD}12`,border:`1px solid ${T.GOLD}30`,borderRadius:5,padding:"4px 12px",fontFamily:M,fontSize:9,color:T.GOLD,cursor:"pointer",fontWeight:600}}>🎲 Surprise Me</button> : null}
              />
              <div className="cs-g2" style={{gap:12}}>
            ...
            <div style={{marginTop:4}}>
              ...
              <div style={{display:"flex",gap:10,justifyContent:"flex-end",alignItems:"center"}}>
                ...
                <button
                  disabled={!canGenerate || !(activeScene.status==="locked"||activeScene.status==="generated") || !sbAllApproved || videoGenerating === activeScene.id}
                  onClick={()=>{ if (canGenerate && sbAllApproved && !videoGenerating) generateVideo(activeScene.id); }}
                  style={{
                    background:canGenerate && (activeScene.status==="locked"||activeScene.status==="generated") && sbAllApproved && !videoGenerating?ACCENT:"#222",
                    border:"none",borderRadius:8,padding:"10px 24px",fontFamily:D,fontWeight:700,fontSize:12,
                    color:canGenerate && (activeScene.status==="locked"||activeScene.status==="generated") && sbAllApproved && !videoGenerating?"#000":"#444",
                    cursor:canGenerate && (activeScene.status==="locked"||activeScene.status==="generated") && sbAllApproved && !videoGenerating?"pointer":"not-allowed",
                    boxShadow:canGenerate && (activeScene.status==="locked") && sbAllApproved && !videoGenerating?`0 4px 20px ${T.GOLD}30`:"none",
                    opacity:canGenerate && (activeScene.status==="locked"||activeScene.status==="generated") && sbAllApproved && !videoGenerating?1:0.5,
                  }}>
                  {!canGenerate ? `🔒 ${currentRole.label} cannot generate`
            ...
            {activeScene.status === "generated" && (() => {
              ...
              const generateTrack = (trackId) => {
                if (!deductCredits(CREDIT_COSTS.generateAudioTrack, `Audio ${trackId} generation`)) return;
              ...
              const generateAllTracks = () => {
                const ungeneratedCount = TRACK_DEFS.filter(td => !(audio.tracks.find(t=>t.id===td.id)?.generated)).length;
                const totalCost = ungeneratedCount * CREDIT_COSTS.generateAudioTrack;
                if (totalCost > 0 && team.creditBalance < totalCost) {
                  notify("error", `Insufficient credits — ${totalCost} required for ${ungeneratedCount} tracks, ${team.creditBalance} remaining.`);
                  return;
                }
                TRACK_DEFS.forEach((td, i) => {
                  if (!(audio.tracks.find(t=>t.id===td.id)?.generated)) {
              ...
              <div style={{background:T.CARD,border:`1px solid ${T.PURPLE}15`,borderRadius:10,padding:0,marginBottom:14,overflow:"hidden",position:"relative",marginTop:14}}>
                ...
                <div style={{padding:"14px 16px 10px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  ...
                  {canEditAudio ? <button onClick={generateAllTracks} disabled={anyGenerating}
                  ...
                  </button> : <span style={{fontFamily:M,fontSize:8,color:"#444"}}>🔒 View only</span>}
                </div>
                      ...
                      <div key={td.id} style={{background:`${td.color}04`,border:`1px solid ${td.color}10`,borderRadius:8,padding:12,marginBottom:6}}>
                          ...
                          <div style={{display:"flex",alignItems:"center",gap:6}}>
                            {track.generated && (
                              <>
                                ...
                                {canEditAudio && <button onClick={()=>generateTrack(td.id)} disabled={track.generating} style={{
                            ...
                            {!track.generated && !track.generating && canEditAudio && (
                        ...
                        {track.generated && (
                          <div>
                            ...
                            {canEditAudio && <div style={{display:"flex",gap:6,marginTop:6}}>
                              <input
        ...
        {view === "review" && (() => {
          ...
          if (!activeTake) return (
            <div style={{textAlign:"center",padding:"60px 20px"}}>
              <div style={{fontSize:40,marginBottom:14,opacity:0.3}}>🎥</div>
              <div style={{fontFamily:D,fontWeight:700,fontSize:16,color:"#555",marginBottom:8}}>Take data unavailable</div>
              <button onClick={()=>setView("console")} style={{background:`${T.GOLD}12`,border:`1px solid ${T.GOLD}30`,borderRadius:6,padding:"8px 18px",fontFamily:D,fontWeight:700,fontSize:12,color:T.GOLD,cursor:"pointer"}}>Go to Console →</button>
            </div>
          );
          ...
          <div>
            ...
            <div style={{display:"flex",gap:10,justifyContent:"flex-end",alignItems:"center"}}>
              ...
              {canReviewFull && !activeTake.approved && (
                <button onClick={() => {
                  setScenes(prev => prev.map(s => s.id !== rScene.id ? s : {
                    ...s,
              ...
              {!canReviewFull && !activeTake.approved && (
                <span style={{fontFamily:M,fontSize:9,color:"#555",background:T.INPUT,borderRadius:6,padding:"8px 14px"}}>🔒 {currentRole.label} cannot approve takes</span>
              )}
              {activeTake.approved && (
        ...
        {view === "timeline" && (() => {
          ...
          const clips = scenes.map(s => {
            ...
            return { sceneId: s.id, number: s.number, title: s.title, duration: dur, hasVideo, hasTake, latestTake, transition: s.screenplay?.transition || "CUT TO:", actName: act?.name || "—", actIdx: Math.max(0, (screenplay.acts||[]).indexOf(act)), approved: latestTake?.approved || false };
          });
