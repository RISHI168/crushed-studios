...
export default function App() {
  ...
  const projectDataStore = useRef({
    "proj_shadows": null, // null = uses live state (it's the default loaded project)
    "proj_monsoon": {
    ...
    <div style={{minHeight:"100vh",background:T.DARK,color:T.TXT,fontFamily:B}}>
      ...
      <header className="cs-header" style={{display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:`1px solid ${T.BORDER}`,backdropFilter:"blur(20px)",position:"sticky",top:0,zIndex:100,background:`${T.DARK}dd`}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <div onClick={()=>{saveCurrentProjectData();setView("dashboard");}} style={{width:28,height:28,borderRadius:6,background:ACCENT,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,fontSize:12,color:"#000",fontFamily:D,cursor:"pointer"}} title="Back to Dashboard">C</div>
          {view !== "dashboard" && (
      ...
      <main className="cs-main" style={{opacity:ok?1:0,transform:ok?"none":"translateY(10px)",transition:"all 0.6s"}}>
                    ...
                    <div style={{padding:"12px 14px"}}>
                      ...
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                        ...
                        <div style={{display:"flex",alignItems:"center",gap:6}}>
                          ...
                          <span style={{fontFamily:M,fontSize:7,color:"#444"}}>{proj.creditUsed > 0 ? proj.creditUsed.toLocaleString()+" cr" : "—"}</span>
                        </div>
                      </div>
        ...
        <div style={{display:"flex",gap:6,marginBottom:20}}>
          ...
          ].concat(scenes.some(s => s.takes?.length > 0) && canReviewAnnotate ? [{id:"review",icon:"🎥",label:"Review Interface",desc:"Review generated output",show:true}] : []).concat(scenes.some(s => s.status === "generated") && canEditTimeline ? [{id:"timeline",icon:"🎞️",label:"Timeline / NLE",desc:"Assemble scenes into film",show:true}] : []).filter(tab=>tab.show).map(tab=>(
