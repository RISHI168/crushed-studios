  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(async () => {
      try {
        ...
        await window.storage.set("crushed-project", JSON.stringify({
      ...
      <header className="cs-header" style={{display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:`1px solid ${T.BORDER}`,backdropFilter:"blur(20px)",position:"sticky",top:0,zIndex:100,background:`${T.DARK}dd`}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:4}}>
            <div style={{width:5,height:5,borderRadius:"50%",background:saveStatus==="saving"?T.GOLD:saveStatus==="saved"?T.GREEN:saveStatus==="error"?T.RED:"#333",
              boxShadow:saveStatus==="saving"?`0 0 6px ${T.GOLD}40`:saveStatus==="saved"?`0 0 6px ${T.GREEN}40`:"none",
              transition:"all 0.3s",
            }}/>
            <span style={{fontFamily:M,fontSize:8,color:saveStatus==="saving"?T.GOLD:saveStatus==="saved"?"#555":saveStatus==="error"?T.RED:"#333"}}>
          ...
          <div style={{position:"relative"}}>
            ...
            {showProjectMenu && (
              <div style={{position:"absolute",top:"100%",right:0,marginTop:4,background:T.CARD,border:`1px solid ${T.BORDER}`,borderRadius:8,padding:4,zIndex:200,minWidth:180,boxShadow:"0 8px 32px #00000060"}}
                ...
                <button onClick={()=>{if(confirm("Reset project to demo data? Your current work will be lost.")){resetProject();}}} style={{display:"flex",alignItems:"center",gap:8,width:"100%",padding:"8px 12px",background:"transparent",border:"none",borderRadius:5,fontFamily:B,fontSize:11,color:T.RED,cursor:"pointer",textAlign:"left"}}
          ...
          <div>
            ...
            <div style={{background:T.CARD,border:`1px solid ${T.BORDER}`,borderRadius:10,overflow:"hidden",marginBottom:14}}>
              ...
              <div style={{display:"flex"}}>
                ...
                {/* ── Shared scroll area ── */}
                <div className="cs-nle-scroll" style={{flex:1,overflowX:"auto"}}>
                  <div style={{width:totalDuration * pxPerSec,minWidth:"100%",position:"relative"}}>
                    ...
                    <div style={{position:"relative",height:56,borderBottom:`1px solid ${T.BORDER}`,padding:"4px 0",opacity:getTlTrackActive("video")?1:0.2,transition:"opacity 0.2s"}}>
                      {clips.map((clip, ci) => {
                        const clipW = clip.duration * pxPerSec;
                        ...
                        const clipLeft = (() => { let l = 0; for(let j=0;j<ci;j++) l += clips[j].duration * pxPerSec; return l; })();
                        return (
                    ...
                    {/* AUDIO TRACKS — Premiere-style: every clip is a visible block */}
                              ...
                              <div key={clip.sceneId}
                                ...
                                {hasAudio && (
                                  <div style={{display:"flex",alignItems:"center",gap:0.5,width:"100%",height:"100%",padding:"3px 2px"}}>
                                    {(track.waveform||[]).slice(0, Math.max(12, Math.floor(clipW / 3))).map((v, wi) => (
                                      <div key={wi} style={{
                                        flex:1, minWidth:1,
                    ...
                    <div style={{position:"relative",height:28,borderBottom:`1px solid ${T.BORDER}10`,opacity:getTlTrackActive("text")?1:0.2,transition:"opacity 0.2s"}}>
                      ...
                      {clips.map((clip, ci) => {
                        const clipW = clip.duration * pxPerSec;
                        const clipLeft = (() => { let l=0; for(let j=0;j<ci;j++) l+=clips[j].duration*pxPerSec; return l; })();
                        return <div key={clip.sceneId} style={{position:"absolute",left:clipLeft+1,top:3,width:clipW-2,height:22,borderRadius:3,border:`1px solid ${T.GOLD}06`}}/>;
                      })}
                      {/* Overlay blocks */}
                      {tlOverlays.map(ovr => (
